...
class proteinImage(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if no_channels==4:
            image_yellow_ch=Image.open(os.path.join(self.img_dir,samp+"_yellow.png"))
            image = Image.merge("RGBA",(image_red_ch,image_green_ch,image_blue_ch,image_yellow_ch))
        else:
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:     
            ...
            for data in dataloaders[phase]:
                ...
                batch_labels.append(labels.cpu().data.numpy())

            epoch_loss = running_loss / len(dataloaders[phase])
            #print(running_corrects)
            #print(running_corrects.double())
            epoch_acc_avg = running_corrects / len(dataloaders[phase])
            #print(batch_labels)
            #print(batch_predicts)
            batch_labels=np.concatenate(batch_labels,axis=0)
            batch_predicts=np.concatenate(batch_predicts,axis=0)
            print(batch_labels)
            print(batch_predicts)
            epoch_acc = f1_score(batch_labels, batch_predicts, average='macro')

            time_elapsed_epoch = time.time() - since_epoch
            print('{} Loss: {:.4f} Avg. Acc: {:.4f} Acc: {:.4f} in {:.0f}m {:.0f}s'.format(
                phase, epoch_loss, epoch_acc_avg,epoch_acc, time_elapsed_epoch // 60, time_elapsed_epoch % 60))
...
model_ft.conv1 = nn.Conv2d(no_channels, 64, kernel_size=7, stride=2, padding=3,
bias=False)
multiGPU = True
use_gpu=True
